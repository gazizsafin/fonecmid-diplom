Дипломный проект профессии «1C-программист: с нуля до middle» (Сафин Г.З.)
...
Полная постановка задачи находится здесь
Решение
Описание дополненного функционала
1. Добавлена подсистема Обслуживание клиентов
Документ Обслуживание клиентов
Работа с заявками осуществляется по следующему алгоритму:
1.	Клиент обращается к менеджеру по телефону и оформляет заявку на выезд специалиста, описывая проблему, требующую решения.
2.	Менеджер подбирает подходящее свободное время и вносит заявку в график.
3.	После планирования заявки телеграм бот автоматически уведомляет специалистов о её появлении.
4.	В оговорённое время специалист прибывает к клиенту (в офис) либо подключается удалённо и выполняет необходимые работы.
5.	По завершении работ специалист получает от клиента подписанный лист учёта рабочего времени либо его скан/фото. В этом документе:
•	перечислены выполненные виды работ;
•	зафиксировано количество часов, подлежащих оплате.
6.	Лист учёта служит официальным подтверждением факта выполнения работ.
7.	Далее специалист вносит в информационную систему следующие данные:
•	перечень выполненных работ;
•	фактическое количество часов, затраченных на каждый вид работ;
•	количество часов, выставляемых к оплате клиенту по каждому виду работ;
•	прикрепляет скан/фото листа учёта рабочего времени.
...
Для работы с данной обработкой создана роль BKM_ИспользованиеОбработкиМассовоеСозданиеАктов. Эта роль подключена к предопределенному профилю пользователей BKM_Бухгалтер.
...
3. Добавлена подсистема BKM_ДобавленныеОбъекты
...
1.	В режиме администратора настроить телеграмм-бот.
...
2.	Кадровик-расчетчик в обязательном порядке должен заполнить справочник Сотрудники фамилиями сотрудника фирмы, а также заполнить регистр сведения УсловияОплатыСотрудников поле ПроцентОтВыполненныхРабот. Поля регистра сведения о графике работы сотрудника и окладу можно заполнить позже.
Перед первым использованием подсистемы Расчет зарплаты.
1.	Заполнить справочник графики работ. Например: 5-Б-1, 5-Б-2, 2-А, 9-А. Для графика отпуск имеется предопределенная запись - Отпуск календарные дни.
2.	С помощью обработки Заполнение графиков работ, заполните соответствующий график. В поле обработку Выходные дни указываются дни недели цифрами (Подряд без пробелов), которые являются выходными. Например: для графика пятидневка поле заполняется значением "5-Б-1".
...
2.	Перед запуском автотеста клиент-тест должен быть закрыт. Документов обслуживание клиентов, реализация товаров и услуг, договоров контрагента не должно быть.
3.	Автотестирование должно проходить в строгой последовательности, так как некоторые тесты опираются на данные созданными другими тестами.
o	
a.	Первым идет запуск автотестирования из файла Создание_документов_Обслуживания_клиента.feature.
o	
b.	Затем - Закрытие_документа_Обслуживание_клиента.feature
o	
c.	Затем - Массовое_создание_актов.feature
o	
d.	Затем - Формирование отчёта Анализ выставленных актов, сравнение с эталоном.
